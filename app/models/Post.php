<?php
    class Post {
        ...
        public function addPostQuery($postArr){
            $this->db->prepare("INSERT INTO posts (post_title, post_image, post_secret_image, post_date, post_status, post_alt_text, post_hover_text) VALUES (:post_title, :post_image, :post_secret_image, :post_date, :post_status, :post_alt_text, :post_hover_text)");
            $this->postLogic($postArr);
            $this->db->execute();
        }

        public function editPostQuery($postArr){
            $this->db->prepare("UPDATE posts SET post_title = :post_title, post_date = :post_date, post_status = :post_status, post_alt_text = :post_alt_text, post_hover_text = :post_hover_text, post_image = :post_image, post_secret_image = :post_secret_image WHERE post_id = :post_id ");
            $this->postLogic($postArr);
        ...
        private function postLogic($postArr) {
            $this->db->bind('post_title', $postArr['post_title']);
            $this->db->bind('post_image', $postArr['post_image']);
            $this->db->bind('post_secret_image', $postArr['post_secret_image']);
