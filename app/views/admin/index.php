...
<table class="table table-sm table-hover table-bordered border-info align-middle fs-6" id="post-table">
  <thead class="table-dark">
    <tr id="theaders">
      <th class="text-center"><input type="checkbox" class="cursor text-center checkbox--styling" id="selectAllBoxes" width="10%"></th>
  ...
  <tbody>
    ...
    ?>
    <tr>
      <td class="text-center"><input style="cursor: pointer;" type='checkbox' class='checkBoxes checkbox--styling' name="checkBoxArray[]" value="<?= $row['post_id']; ?>"></td>
      <td scope="row" class="fw-bolder"><?= $row['post_id']; ?></th>
      <td class="fs-5"><?= $row['post_title']; ?></td>
      <td><img width="auto" height="90rem" src="<?= $row['img_link']; ?>" alt=""></td>
      <td><img width="auto" height="90rem" src="<?= $row['secret_img_link']; ?>" alt=""></td>
      <td class="fs-5"><?= $row['post_date']; ?></td>
      <td><?= $row['post_status']; ?></td>
      <td><form action="<?= $row['form_action']; ?>" method="POST"><input type="hidden" name="id" value="<?= $row['post_id'] ;?>"><input class="btn btn-outline-dark mx-2" type="submit" name="published" value="Publish"></form></td> 
      <td><form action="<?= $row['form_action']; ?>" method="POST"><input type="hidden" name="id" value="<?= $row['post_id'] ;?>"><input class="btn btn-outline-dark mx-2" type="submit" name="draft" value="Draft"></form></td>
      <td><a href="<?= $row['edit_link']; ?>" class="btn btn-outline-dark mx-2">Edit</a></td>
      <td><a href="<?= $row['delete_link']; ?>" class="btn btn-outline-dark mx-2">Delete</a></td>
      <td><a href="<?= $row['post_link']; ?>" class="btn btn-outline-dark mx-2">View</a></td>
    </tr>   
    <?php endforeach; ?>
  </tbody>
</table>
